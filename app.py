...
def init_db():
    ...
    if PG:
        ...
        ''')
        # SQLite migrations
        for col, typ in [('goods_payment_percent','REAL DEFAULT 0'),('docs_received_date','DATE'),('docs_notes','TEXT')]:
...
def index():
    ...
    for a in agreements:
        ...
        result.append({'data':a,'goods':goods,'goods_total':goods_total,
            ...
            'delivery_total':delivery_total,'days_until_eta':days_until(a['eta'])})
    ...
    if aid:
        ...
        if PG:
            cur = execute(conn, '''INSERT INTO agreements (agreement_number,container_count,delivery_terms,
                ...
                goods_payment_percent,docs_received,docs_received_date,docs_notes,record_number,notes)
