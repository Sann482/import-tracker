...
def init_db():
    ...
    if PG:
        ...
        for col, typ in [('eta_sklad','DATE'),('goods_payment_percent','REAL DEFAULT 0'),('docs_received_date','DATE'),('docs_notes','TEXT')]:
            try: cur.execute(f"ALTER TABLE agreements ADD COLUMN IF NOT EXISTS {col} {typ}")
            except: pass
...
def index():
    ...
    return render_template('index.html', agreements=result, status_filter=status_filter, search=search,
        ...
        f_pol=f_pol, f_pod=f_pod, f_terms=f_terms, vis_cols=get_visible_cols(), bg_color=get_bg_color(), theme_vars=get_theme_vars())
...
def new_agreement():
    ...
    return render_template('form.html', agreement=None, goods=[], opts=opts, theme_vars=get_theme_vars(), bg_color=get_bg_color())
...
def edit_agreement(aid):
    ...
    conn.close()
    opts = {c: get_options(c) for c in ['pol','pod','shipping_line','supplier','container_count','good_name']}
    return render_template('form.html', agreement=agreement, goods=goods, opts=opts, theme_vars=get_theme_vars(), bg_color=get_bg_color())
...
def view_agreement(aid):
    ...
    return render_template('detail.html', a=a, goods=goods,
        ...
        delivery_total=delivery_total, days_until_eta=days_until(a['eta_sklad'] or a['eta'], bg_color=get_bg_color()), theme_vars=get_theme_vars())
...
def settings_page():
    opts = {c: get_options(c) for c in ['pol','pod','shipping_line','supplier','container_count','good_name']}
    return render_template('settings.html', opts=opts, vis_cols=get_visible_cols(), bg_color=get_bg_color(), theme_vars=get_theme_vars())
...
def dashboard():
    ...
    return render_template('dashboard.html', total=total, archived=archived, overdue=overdue, soon=soon,
        ...
        recent=recent, suppliers=suppliers, days_until=days_until, theme_vars=get_theme_vars(), bg_color=get_bg_color())

def get_theme_vars():
    """Return CSS :root vars string for current theme - injected directly into HTML"""
    PRESETS = {
        'light': {'bg':'#f6f6f4','bg2':'#ffffff','bg3':'#f0f0ee','border':'#e8e8e4','border2':'#d0d0cc','text':'#18181b','muted':'#71717a','hint':'#a1a1aa'},
        'warm':  {'bg':'#fffbf5','bg2':'#fff8ef','bg3':'#fef3e2','border':'#ede0c8','border2':'#d4c5a9','text':'#2c1a00','muted':'#8a6a3a','hint':'#b09060'},
        'dark':  {'bg':'#141414','bg2':'#1e1e1e','bg3':'#282828','border':'#333333','border2':'#444444','text':'#e8e8e8','muted':'#888888','hint':'#555555'},
        'slate': {'bg':'#f8fafc','bg2':'#ffffff','bg3':'#f1f5f9','border':'#e2e8f0','border2':'#cbd5e1','text':'#0f172a','muted':'#64748b','hint':'#94a3b8'},
        'cream': {'bg':'#fdf6e3','bg2':'#fdf8ef','bg3':'#f5edd6','border':'#e8dfc8','border2':'#d4c9a8','text':'#433422','muted':'#8a7a5a','hint':'#b0a07a'},
        'night': {'bg':'#0f172a','bg2':'#1e293b','bg3':'#293548','border':'#334155','border2':'#475569','text':'#e2e8f0','muted':'#94a3b8','hint':'#475569'},
    }
    try:
        conn = get_db()
        row = fetchone(conn, "SELECT value FROM settings WHERE key='theme'")
        conn.close()
        if not row:
            # Default theme vars to prevent flash before user saves a theme
            return "--bg:#f6f6f4;--bg2:#ffffff;--bg3:#f0f0ee;--border:#e8e8e4;--border2:#d0d0cc;--text:#18181b;--muted:#71717a;--hint:#a1a1aa;--radius:8px;--sans:-apple-system,BlinkMacSystemFont,'Segoe UI','Helvetica Neue',Arial,sans-serif"
        t = json.loads(row['value'])
        vars_list = []
        preset = PRESETS.get(t.get('preset','light'), PRESETS['light'])
        ...
        if t.get('colors'):
            for k,v in t['colors'].items():
                vars_list.append(f'{k}:{v}')
        if t.get('radius'):
            vars_list.append(f"--radius:{t['radius']}")
        if t.get('size'):
            vars_list.append(f"--size:{t['size']}")
        font = t.get('font', 'Inter')
...
def get_bg_color():
    """Return background color for current theme"""
    PRESETS = {
        'light':'#f6f6f4','warm':'#fffbf5','dark':'#141414',
        'slate':'#f8fafc','cream':'#fdf6e3','night':'#0f172a',
    }
    try:
        conn = get_db()
        row = fetchone(conn, "SELECT value FROM settings WHERE key='theme'")
        ...
        if not row:
            return '#f6f6f4'
        t = json.loads(row['value'])
        # Check custom bg color first
        colors = t.get('colors', {})
        if '--bg' in colors:
            return colors['--bg']
        return PRESETS.get(t.get('preset','light'), '#f6f6f4')
    except:
        return '#f6f6f4'
    ...
    if row:
        ...
        if t.get('colors'):
            for k,v in t['colors'].items():
                vars_css.append(f'  {k}:{v};')
        if t.get('radius'):
            vars_css.append(f"  --radius:{t['radius']};")
        if t.get('font'):
            vars_css.append(f"  --sans:'{t['font']}',sans-serif;")
        if t.get('size'):
            vars_css.append(f"  font-size:{t['size']};")
    else:
...
def import_data():
    ...
    return render_template('import_export.html', theme_vars=get_theme_vars(), bg_color=get_bg_color())

@app.route('/import-success')
def import_success():
    agreements = request.args.get('agreements', 0)
    goods = request.args.get('goods', 0)
    return render_template('import_export.html', success=True, agreements=agreements, goods=goods, theme_vars=get_theme_vars(), bg_color=get_bg_color())
