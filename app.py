...
def index():
    ...
    return render_template('index.html', agreements=result, status_filter=status_filter, search=search,
        ...
        f_pol=f_pol, f_pod=f_pod, f_terms=f_terms, vis_cols=get_visible_cols(), theme_vars=get_theme_vars())
...
def new_agreement():
    ...
    return render_template('form.html', agreement=None, goods=[], opts=opts, theme_vars=get_theme_vars())
...
def edit_agreement(aid):
    ...
    conn.close()
    opts = {c: get_options(c) for c in ['pol','pod','shipping_line','supplier','container_count','good_name']}
    return render_template('form.html', agreement=agreement, goods=goods, opts=opts, theme_vars=get_theme_vars())
...
def view_agreement(aid):
    ...
    return render_template('detail.html', a=a, goods=goods,
        ...
        delivery_total=delivery_total, days_until_eta=days_until(a['eta_sklad'] or a['eta']), theme_vars=get_theme_vars())
...
def settings_page():
    opts = {c: get_options(c) for c in ['pol','pod','shipping_line','supplier','container_count','good_name']}
    return render_template('settings.html', opts=opts, vis_cols=get_visible_cols(), theme_vars=get_theme_vars())
...
def dashboard():
    ...
    return render_template('dashboard.html', total=total, archived=archived, overdue=overdue, soon=soon,
        ...
        recent=recent, suppliers=suppliers, days_until=days_until, theme_vars=get_theme_vars())
...
def import_data():
    ...
    return render_template('import_export.html', theme_vars=get_theme_vars())

@app.route('/import-success')
def import_success():
    agreements = request.args.get('agreements', 0)
    goods = request.args.get('goods', 0)
    return render_template('import_export.html', success=True, agreements=agreements, goods=goods, theme_vars=get_theme_vars())
